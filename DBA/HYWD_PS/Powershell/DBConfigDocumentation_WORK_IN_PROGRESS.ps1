...
$DirectoryToSaveTo='C:\health\SQL_Server\'
...
$Servers=[xml] @'
<serverList>
      <server  version="2012" name="sqldba" />
      <server  version="2012" name="hitsqlvm1" />
      <server  version="2012" name="scmmavmtrn" />
</serverList>
'@
...
foreach ($server in $servers.serverlist.server)
      ...
      if  ($server.version -eq 2012) {$SQL=$SQL2012}
