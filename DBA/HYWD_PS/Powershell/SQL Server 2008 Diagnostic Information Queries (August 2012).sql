...
-- Get VLF Counts for all databases on the instance (Query 17)
...
-- Get CPU utilization by database (adapted from Robert Pearl)  (Query 19)
WITH DB_CPU_Stats
AS
...
-- Isolate top waits for server instance since last restart or statistics clear  (Query 21)
WITH Waits AS
...
-- SQL Server Process Address space info  (Query 27) 
...
-- Memory Grants Pending value for current instance  (Query 30)
...
-- I/O Statistics by file for the current database  (Query 34)
...
-- Get VLF count for transaction log for the current database,  (Query 35)
...
-- Top cached queries by Execution Count (SQL Server 2008)  (Query 36)
...
-- Top Cached SPs By Execution Count (SQL 2008) (SQL 2008 R2 SP1 only) (Query 37)
...
-- Top Cached SPs By Avg Elapsed Time (SQL 2008)  (Query 38)
...
-- Top Cached SPs By Avg Elapsed Time with execution time variability   (Query 39)
...
-- Top Cached SPs By Total Worker time (SQL 2008). Worker time relates to CPU cost  (Query 40)
...
-- Top Cached SPs By Total Logical Reads (SQL 2008). Logical reads relate to memory pressure  (Query 41)
...
-- Top Cached SPs By Total Physical Reads (SQL 2008). Physical reads relate to disk I/O pressure  (Query 42)
...
-- Top Cached SPs By Total Logical Writes (SQL 2008)  (Query 43) 
...
-- Lists the top statements by average input/output usage for the current database  (Query 44)
...
-- Possible Bad NC Indexes (writes > reads)  (Query 45)
...
-- Missing Indexes for current database by Index Advantage  (Query 46)
...
-- Find missing index warnings for cached plans in the current database  (Query 47)
...
-- Breaks down buffers used by current database by object (table, index) in the buffer cache  (Query 48)
-- Note: This query could take some time on a busy instance
...
-- Get Table names, row counts, and compression status for clustered index or heap  (Query 49)
...
-- When were Statistics last updated on all indexes?  (Query 50)
...
-- Get fragmentation info for all indexes above a certain size in the current database  (Query 51) 
...
--- Index Read/Write stats (all tables in current DB) ordered by Reads  (Query 52)
...
--- Index Read/Write stats (all tables in current DB) ordered by Writes  (Query 53)
SELECT OBJECT_NAME(s.[object_id]) AS [ObjectName], i.name AS [IndexName], i.index_id,
...
-- Look at recent Full backups for the current database (Query 54)
